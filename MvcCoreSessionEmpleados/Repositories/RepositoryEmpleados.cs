...
namespace MvcCoreSessionEmpleados.Repositories
{
    public class RepositoryEmpleados
    {
        ...
        public async Task<List<Empleado>> GetEmpleadosNotSessionAsync(List<int> empleadosIds)
        {
            var consulta =
                from datos in this.context.Empleados
                where !empleadosIds.Contains(datos.IdEmpleado)
                select datos;

            return await consulta.ToListAsync();
        }

        public async Task<List<Empleado>> GetEmpleadosSessionAsync(List<int> empleadosIds)
        {
            var consulta =
                from datos in this.context.Empleados
                where empleadosIds.Contains(datos.IdEmpleado)
